...
public class Intelligent_Score_ShowAll extends JDialog{
	JLabel jl_title=new JLabel("智   育   成   绩   管   理");
...
//	JButton jb_fail=new JButton("不及格");
	...
	int count=0;
	private int row;
	public String temp_intelligent_Name;
	public  String name;
	public double score=0;
	public Intelligent_Score_ShowAll() {
...
//		jp.add(jb_fail,new XYConstraints(372, 15, 88, 37));
	...
	public void UpdateTable() {
		...
		try {
			while(rs.next()) {
				...
				value.add(rs.getString("intelligent_name"));
				value.add(rs.getString("intelligent_score"));
				row.add(value);
			}
	...
	public void score_order(){
		...
		if(count%2==1) {//对成绩升序
			jb_sort.setText("降 序");
			sql = "select * from student_intelligent order by intelligent_score asc";
			UpdateTable();
		}else {//对成绩降序
			jb_sort.setText("升 序");
			sql = "select * from student_intelligent order by intelligent_score desc";
			UpdateTable();
		}
	...
	public void score_require() {
		...
		Intelligent_score_require update=new Intelligent_score_require();
...
//	}
	...
	public void getRow() {
		...
		name = table.getValueAt(row, 1).toString();      // 获取姓名
		temp_intelligent_Name = table.getValueAt(row, 2).toString();     //
		score=Double.valueOf(table.getValueAt(row, 3).toString());
		System.out.println(name);
		System.out.println(temp_intelligent_Name);
	}
	//点击删除按钮
	public void deleteScore() {
		...
		if(row!=-1) {
			...
			if(i==0) {
				dao.getUpdate("delete from student_intelligent where intelligent_name='"+temp_intelligent_Name.trim()+"' and student_name='"+name+"'");
				new Intelligent_Score_Update(name,score);
				sql="select * from student_intelligent";
				UpdateTable();
			}
		}else {
	...
	public void UpdateScore() {
		getRow();
		if(row!=-1) {
			Intelligent_Score_Update update=new Intelligent_Score_Update(name,temp_intelligent_Name);
