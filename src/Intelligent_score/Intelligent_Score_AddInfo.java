...
public class Intelligent_Score_AddInfo extends JDialog{
	Font f=new Font("微软",Font.BOLD,25);
	Font font=new Font("微软",Font.PLAIN,18);
	JLabel jl_title=new JLabel("学  生 智 育 成  绩  添  加");
	...
	public Intelligent_Score_AddInfo() {     // 设置框体
	...
	public void init() {
		jl_title.setFont(f);
		jl_title.setBounds(new Rectangle(108,10,300,30));
		cont.add(jl_title);
	...
	public void Sumbitscore() {
		...
		if(jtf_student_name.getText().equals("")) {
			...
			if (isName) {
				...
				rs = dao.getRs("select * from student_intelligent where student_name='"+String.valueOf(jtf_student_name.getText().trim())+"'");  // and moral_name='"+jcb_moral_score_name.getText().trim()+"'
				try {
					while(rs.next()) {
						String student_moralname = rs.getString("intelligent_name");
				...
				if(isMoral) {
					...
					int update = dao
							.getUpdate("insert into student_intelligent (student_number,student_name,intelligent_name,intelligent_score) values('" +temp.trim()+ "','" + jtf_student_name.getText().trim() + "','"
									+ String.valueOf(jcb_moral_score_name.getText()).trim() + "','" + jtf_moral_score.getText().trim()
									+ "')");
					rs=dao.getRs("select * from student_intelligent_score where student_name='"+String.valueOf(jtf_student_name.getText().trim())+"'");
					boolean flag=false;
					int insert_flag=0;
					int cur_num=0;
					double cur_sum=0;
					...
					if(flag) {    //如果原表中存在该同学信息
						...
						int res = dao.getUpdate("update student_intelligent_score set total_num="+cur_num+",sum="+cur_sum+" where student_name='"+jtf_student_name.getText().trim()+"'");
						if(res==1) {
...
//							dispose();
						...
						int res = dao.getUpdate("insert into student_intelligent_score (student_number,student_name,total_num,sum) values('" +
