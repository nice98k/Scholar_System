...
public class Moral_score_require extends JDialog {
	...
	public void UpdateRecord() {
		...
		try {
			...
			while (rs.next()) {
				...
				vec.add(rs.getString("moral_name").trim());
				vec.add(rs.getString("moral_score").trim());
				model.addRow(vec);
			}
	...
	public void Row() {
		...
		if (intRow == -1)
			...
			Moral_Name = model.getValueAt(intRow, 2).toString();
			score=Double.valueOf(model.getValueAt(intRow, 3).toString().trim());
//			System.out.println(s_name+"    "+Moral_Name);
	}
	...
	public void deleteStudent() {
		 dao.getUpdate("delete from student_moral where student_name='" + s_name + "' and moral_name='"+Moral_Name+"'");
		 new MoralScoreUpdate(s_name,score);
	}
		...
		public void actionPerformed(ActionEvent e) {
			...
			if(soucre==jb_number) {
				...
				if(jtf_number.getText().trim().equals("")) {
					...
					sql="select * from student_moral where student_number='"+jtf_number.getText().trim()+"'";
					UpdateRecord();
				}
			...
			if(soucre==jb_name) {
				...
				if(jtf_name.getText().trim().equals("")) {
					...
					sql="select * from student_moral where moral_name like'"+"%"+jtf_name.getText().trim()+"%"+"'";
					UpdateRecord();
				}
			...
			if(soucre==jb_update) {
				Row();
				if(intRow!=-1) {
					MoralScoreUpdate update=new MoralScoreUpdate(s_name,Moral_Name);
					if(update.isUpdate) {
						sql = "select * from student_moral";
						UpdateRecord();
					}
