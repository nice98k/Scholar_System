...
public class MoralScoreShowAll extends JDialog{
...
//		jp.add(jb_fail,new XYConstraints(372, 15, 88, 37));
	...
	public void UpdateTable() {
		...
		try {
			while(rs.next()) {
				...
				value.add(rs.getString("moral_name"));
				value.add(rs.getString("moral_score"));
				row.add(value);
			}
	...
	public void score_order(){
		...
		if(count%2==1) {//对成绩升序
			jb_sort.setText("降 序");
			sql = "select * from student_moral order by moral_score asc";
			UpdateTable();
		}else {//对成绩降序
			jb_sort.setText("升 序");
			sql = "select * from student_moral order by moral_score desc";
			UpdateTable();
		}
	...
	public void score_require() {
		...
		Moral_score_require update=new Moral_score_require();
	...
	public void Score_fail() {
		...
		int failsum=0;
		//获取总人数
		ResultSet rs = dao.getRs("select * from student_moral");
		try {
			while(rs.next()) {
				sum++;
			}
	...
	public void getRow() {
		...
		Moral_Name = table.getValueAt(row, 2).toString();     //
	...
	public void deleteScore() {
		...
		if(row!=-1) {
			...
			if(i==0) {
				dao.getUpdate("delete from student_moral where moral_name='"+Moral_Name.trim()+"' and student_name='"+name+"'");
				new MoralScoreUpdate(name,score);
				sql="select * from student_moral";
				UpdateTable();
			}
		}else {
	...
	public void UpdateScore() {
		getRow();
		if(row!=-1) {
			MoralScoreUpdate update=new MoralScoreUpdate(name,Moral_Name);
			if(update.isUpdate) {
				sql="select * from student_moral";
				UpdateTable();
			}
