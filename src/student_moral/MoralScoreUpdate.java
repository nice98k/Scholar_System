...
public class MoralScoreUpdate extends JDialog{
	JLabel jl_title=new JLabel("学  生 德 育 成  绩  修  改");
	...
	public MoralScoreUpdate(String s_name,double score) {
	...
	public void Delete_final(String s_name,double score) {
		...
		ResultSet rs=null;
		try {
			rs=dao.getRs("select * from student_moral_score where student_name='"+s_name+"'");
			while(rs.next()) {
			...
			int res=dao.getUpdate("update student_moral_score set total_num="+cur_total+",sum="+cur_sum+" where student_name='"+s_name+"'");
			if(res==1) {
...
//			System.out.println(cur_sum);
			int res=dao.getUpdate("update student_moral_score set sum="+cur_sum+" where student_name='"+s_name+"'");
			if(res==1) {
	...
	public MoralScoreUpdate(String name,String valueName) {
		this.name=name;
...
//				moral_name.addItem(course_name);
//			}
//		} catch (SQLException e) {
//			// TODO Auto-generated catch block
//			e.printStackTrace();
...
//	}
	...
	public void ReturnScore() {
		ResultSet rs = dao.getRs("select * from student_moral where student_name='"+name+"' and moral_name='"+valueName+"'");
		try {
			while(rs.next()) {
				String temp_student_name = rs.getString("student_name");
				String temp_moral_name = rs.getString("moral_name");
				String temp_score = rs.getString("moral_score");
				jtf_student_name.setText(temp_student_name);
				jtf_student_name.setEditable(false);
				moral_name.setText(temp_moral_name);
	...
	public void UpdateScore() {
		Update();
		//在不修改学生姓名时
//		if(name.equals(jtf_student_name.getText())) {
//			Update();
//		}else {
...
//		}
	...
	public void Update() {
		...
		try {
			...
			while(rs.next()) {
				pre_score=rs.getDouble("moral_score");
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		...
		int update = dao.getUpdate("update student_moral set moral_score='"+jtf_moral_score.getText().trim()+"' where student_name='"+name+"' and moral_name='"+String.valueOf(moral_name.getText().trim())+"'");
		Update_final(name,moral_name.getText().trim(),pre_score);
		if(update==1) {
			isUpdate=true;
