...
public class StudentShowAll extends JDialog{
	...
	String[] arrField = { "学号", "姓名", "邮箱","学院", "专业", "班级" };
	...
	public void UpdateRecord() {
		...
		try {
			...
			while (rs.next()) {
				...
				vec.add(rs.getString("student_email").trim());
				vec.add(rs.getString("student_collage").trim());
				vec.add(rs.getString("student_major").trim());
				vec.add(rs.getString("student_class").trim());
				model.addRow(vec);
			}
	...
	public void Row() {
		...
		if (intRow == -1)
			...
			Tablename = model.getValueAt(intRow, 1).toString();
	}
	//删除
	public void deleteStudent() {
		 dao.getUpdate("delete from student_stu where student_number='" + find + "'");
	}
		...
		public void actionPerformed(ActionEvent e) {
			...
			if(soucre==jb_number) {
				...
				if(jtf_number.getText().trim().equals("")) {
					...
					sql="select * from student_stu where student_number='"+jtf_number.getText().trim()+"'";
					UpdateRecord();
				}
			...
			if(soucre==jb_name) {
				...
				if(jtf_name.getText().trim().equals("")) {
					...
					sql="select * from student_stu where student_name like'"+"%"+jtf_name.getText().trim()+"%"+"'";
					UpdateRecord();
				}
			...
			if(soucre==jb_update) {
				Row();
				if(intRow!=-1) {
					StudentUpdate update=new StudentUpdate(find,Tablename);
					if(update.isUpdate) {
						sql = "select * from student_stu";
						UpdateRecord();
					}
